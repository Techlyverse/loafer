...
class GroupChatScreen extends StatefulWidget {
  const GroupChatScreen({super.key, this.roomModel, required this.roomId});
...
class _GroupChatScreenState extends State<GroupChatScreen> {
  late final RoomModel feedModel;
  ...
  void _updateChange(DocumentChange member) {
    ...
    MemberModel memberModel =
        members[memberId]!.copyWith(isActive: member.doc['isActive']);
    members[memberId] = memberModel;
    setState(() {});
  }
  ...
  Future<void> getRoomInfo() async {
    if (widget.roomModel != null) {
      setState(() {
        feedModel = widget.roomModel!;
      });
    } else {
      final DocumentSnapshot snapshot =
          await FirebaseHelper.roomDoc(widget.roomId).get();
      feedModel = RoomModel.fromJson(snapshot.data()! as Map<String, dynamic>);
      setState(() {});
    }
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Row(
          ...
          children: [
            CircleAvatar(
              backgroundImage: CachedNetworkImageProvider(feedModel.roomAvatar),
            ),
            SizedBox(width: 10),
            Expanded(
              child: Column(
